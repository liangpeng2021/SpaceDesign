Shader "OXRTK/Sprite/FilletFormSprite" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                clip(FrameWorldH( i.uv0 , objScale , _Thickness_var , _filletScale_var , _solid_var ) - 0.5);
            	
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float4 _Color_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Color );
                float finalForm = (_MainTex_var.a*_Color_var.a*i.vertexColor.a); // A
                float3 emissive = ((_MainTex_var.rgb*_Color_var.rgb*i.vertexColor.rgb)*finalForm);
                float3 finalColor = emissive;
                float _opacity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _opacity );
                return fixed4(finalColor,(finalForm*_opacity_var));
            }
            ENDCG
        }
    }
