...
{
    /// <summary>
    /// The class for pressable button base ui interaction receiver. <br>
    /// 可按压按键基类接收端近场交互的类。<br>
    /// Pressable direction is position direction on Z axis.<br>
    /// 按压方向需要为z轴正向。
    /// </summary>
    public class ButtonBaseUIReceiver : UiInteractionPointerHandler
    {
        /// <summary>
        ...
        protected Vector3 m_HandlerStartPosition;
        //在按压方向上的最大按压距离（> 0）
        protected float m_DistanceThreshold;
        //前一次按压方向上距离，用于判断按压方向
        protected float m_PreviousDistance = float.NegativeInfinity;
        //用于判断计算是否进行按压计算
        protected bool m_UpdatePress = false;
        //当按键按到底部时，参数为true，当交互位置仍在collider内但运动方向为local z负向时(抬起时)按键被锁，按压不弹起，只有当手脱离collider或超过交互距离或切换交互物体时按键才会重新弹起
        protected bool m_IsLockedDir = false;
        //在一次按压操作中第一次按到底部时置为并保持true
        protected bool m_IsClicked = false;

        protected float m_PrevDis = float.PositiveInfinity;
        ...
        protected Vector3 GetLocalScale(Transform t, Vector3 worldScale)
        {
